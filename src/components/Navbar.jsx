      ...
      >
        ...
        >
          {navLinks.map((link)=>(
            <li 
            key={link.id}
            className={`${active === link.title ? "text-white" : "text-secondary"} hover:text-white text-[18px] cursor-pointer`}
        ...
        >
          {navLinks.map((link)=>(
            <li 
            key={link.id}
            className={`${active === link.title ? "text-white" : "text-secondary"} font-medium font-poppins  text-[16px] cursor-pointer`}
            onClick={()=>{
              setToggle(!toggle)
